...
the canonical Agent Plugins layout from the pinned src and validates
the result with `flox-agent check-plugin` when the binary is
available (see [ADR 0002](../decisions/0002-optional-check-phase.md)).
...
`checks` covering every package plus the `layout` and
`override-hook` assertions.
